...
@click.option("-d", "dictionary_id", required=True, default=16, show_default=True, type=int, help="Dictionary id (0..16)")
...
def main(calibration_file, marker_length, dictionary_id, marker_id, cam_id, width, height, fps, picam):
    try:
        ...
        tracker = TrackingService(pose_estimator=pose_estimator, camera = camera, target=target_marker, detector= detector, calibration=calibration_data)

        while True:
            res = tracker.track_once()
            logger.info(f"res: {res}")
    except Exception as e:
        logger.error(f"Erreur: ", exc_info=True)
