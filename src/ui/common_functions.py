...
from infrastructure.communication.mavlink import MavlinkConnectionParams, DroneMavlinkUDPConnector, DroneMavlinkSerialConnector
from infrastructure.persistence.configuration_models import DroneConnectionConfiguration, CameraConfiguration
from simulation.gazebo_camera import GazeboCamera


def build_camera(camera_config: CameraConfiguration, calibration_data: CalibrationData, use_simulated_cam: Optional[bool] = False) -> Camera:
    ...
    if camera_config.use_picamera:
        ...
        return PiCameraAdapter(width=calibration_data.camera_width, height=calibration_data.camera_height, fps=camera_config.fps)

    return OpenCVCamera(source=camera_config.id, width=calibration_data.camera_width, height=calibration_data.camera_height, fps=camera_config.fps)
